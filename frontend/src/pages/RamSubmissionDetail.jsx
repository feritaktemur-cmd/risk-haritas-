...
import { Loader2, ArrowLeft, AlertTriangle, MapPinned, Users, CheckCircle2, Circle, Percent, ListChecks, ChevronDown, ChevronRight } from "lucide-react";
...
export default function RamSubmissionDetail() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`${API}/ram/risk-map/submissions/${submissionId}`, { headers: h });
      setData(res.data);
    } catch (err) {
  ...
  if (!ready) {
    return (
      <div className="grid min-h-screen place-items-center bg-[#0b1120]" data-testid="ramdetail-loading">
  ...
  return (
    <div className="min-h-screen bg-[#0b1120] bg-[radial-gradient(60rem_40rem_at_80%_-10%,rgba(16,185,129,0.15),transparent),radial-gradient(50rem_30rem_at_-10%_20%,rgba(99,102,241,0.10),transparent)]">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div onClick={() => navigate("/ram/modules")} role="button" tabIndex={0} data-testid="brand-home-link" className="flex cursor-pointer items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-lg font-extrabold text-white" data-testid="ramdetail-title">Okul Gönderim Detayı</h1>
            </div>
          </div>
          <button onClick={() => navigate("/ram/risk-map")} data-testid="ramdetail-back-btn" className="inline-flex items-center gap-2 rounded-full bg-white/[0.06] px-4 py-2 text-sm font-semibold text-slate-200 ring-1 ring-white/10 transition hover:bg-white/[0.1]">
            <ArrowLeft size={15} /> Risk Haritası Gönderimleri
          </button>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        {error ? (
          <div data-testid="ramdetail-error" className="flex items-start gap-2 rounded-xl bg-rose-500/10 p-4 text-sm text-rose-300 ring-1 ring-rose-400/20">
            <AlertTriangle size={16} className="mt-0.5 shrink-0" /> <span>{error}</span>
          </div>
        ) : data ? (
          <div data-testid="ramdetail-content">
            <div className="mb-8">
              <h2 className="text-2xl font-extrabold text-white" data-testid="ramdetail-school-name">{data.school_name || "—"}</h2>
              <p className="mt-1 text-sm text-slate-400" data-testid="ramdetail-meta">
            ...
            <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              <StatCard icon={Users} label="Toplam Öğrenci" value={data.summary.total_students} tone="text-slate-300" testid="ramdetail-stat-total" />
              <StatCard icon={CheckCircle2} label="Formu Tamamlanan" value={data.summary.completed} tone="text-emerald-400" testid="ramdetail-stat-completed" />
              <StatCard icon={Circle} label="Girilmeyen" value={data.summary.not_entered} tone="text-slate-500" testid="ramdetail-stat-not-entered" />
              <StatCard icon={Percent} label="Tamamlanma Oranı" value={`%${data.summary.completion_rate}`} tone="text-indigo-400" testid="ramdetail-stat-rate" />
              <StatCard icon={ListChecks} label="Toplam Risk İşaretlemesi" value={data.summary.total_marks} tone="text-rose-400" testid="ramdetail-stat-marks" />
            </div>
            ...
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="inline-flex rounded-xl bg-white/[0.05] p-1 ring-1 ring-white/10">
                <button onClick={() => setDomainSort("prevalence")} data-testid="ramdetail-domain-sort-prevalence" className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${domainSort === "prevalence" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:text-white"}`}>Yaygınlığa göre</button>
                <button onClick={() => setDomainSort("order")} data-testid="ramdetail-domain-sort-order" className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${domainSort === "order" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:text-white"}`}>Alan sırasına göre</button>
              </div>
            </div>
            <div className="mb-8 space-y-2.5" data-testid="ramdetail-domains">
            ...
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="inline-flex rounded-xl bg-white/[0.05] p-1 ring-1 ring-white/10">
                <button onClick={() => setCatSort("density")} data-testid="ramdetail-cat-sort-density" className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${catSort === "density" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:text-white"}`}>Yoğunluğa göre</button>
                <button onClick={() => setCatSort("form")} data-testid="ramdetail-cat-sort-form" className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${catSort === "form" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:text-white"}`}>Form sırasına göre</button>
              </div>
            </div>
            <div className="mb-8 space-y-2.5" data-testid="ramdetail-categories">
            ...
            <div className="space-y-3" data-testid="ramdetail-classes">
              {data.classes.length === 0 ? (
                ...
                return (
                  <div key={c.class_name} className="rounded-2xl border border-white/10 bg-white/[0.03]" data-testid={`ramdetail-class-${c.class_name}`}>
                    <button
                      onClick={() => setExpanded((p) => ({ ...p, [c.class_name]: !open }))}
                      data-testid={`ramdetail-class-toggle-${c.class_name}`}
