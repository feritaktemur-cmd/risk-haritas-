...
export default function RamRiskMap() {
  ...
  const load = useCallback(async () => {
    ...
    if (!h) { navigate("/ram/login", { replace: true }); return null; }
    try {
      const res = await axios.get(`${API}/ram/risk-map/submissions`, { headers: h });
  ...
  // Only the RAM's own (scoped) districts appear here — derived from scoped results.
  ...
  if (!ready) {
    return (
      <div className="grid min-h-screen place-items-center bg-[#0b1120]" data-testid="ramrisk-loading">
  ...
  return (
    <div className="min-h-screen bg-[#0b1120] bg-[radial-gradient(60rem_40rem_at_80%_-10%,rgba(16,185,129,0.15),transparent),radial-gradient(50rem_30rem_at_-10%_20%,rgba(99,102,241,0.10),transparent)]">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <h2 className="mb-1 text-base font-bold text-white">Risk Haritası Gönderimleri</h2>
        <p className="mb-6 text-xs text-slate-400">Sorumlu olduğunuz ilçelerdeki okulların RAM'a gönderdiği toplu (anonim) Risk Haritası snapshot'ları. Tüm sürümler geçmişiyle listelenir.</p>

        {error ? (
          <div data-testid="ramrisk-error" className="flex items-start gap-2 rounded-xl bg-rose-500/10 p-4 text-sm text-rose-300 ring-1 ring-rose-400/20">
          ...
          <>
            <div className="mb-4 flex flex-wrap items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  data-testid="ramrisk-search"
                  placeholder="Okul ara"
                  className="rounded-xl border border-white/10 bg-white/[0.04] py-2.5 pl-9 pr-3 text-sm text-white outline-none placeholder:text-slate-500 focus:border-emerald-400/60"
                />
              </div>
              <select value={district} onChange={(e) => setDistrict(e.target.value)} data-testid="ramrisk-district-filter" className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-white outline-none focus:border-emerald-400/60">
                <option value="">Tüm Sorumlu İlçeler</option>
                {districts.map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} data-testid="ramrisk-status-filter" className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-white outline-none focus:border-emerald-400/60">
            ...
            {submissions.length === 0 ? (
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-10 text-center text-sm text-slate-400" data-testid="ramrisk-empty">
                Sorumlu ilçelerinizde henüz RAM'a gönderilmiş Risk Haritası bulunmuyor.
              </div>
            ) : (
              <div className="overflow-x-auto rounded-2xl border border-white/10">
                <table className="w-full min-w-[980px] text-left text-sm" data-testid="ramrisk-table">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filtered.length === 0 ? (
                      ...
                      filtered.map((s) => (
                        <tr key={s.submission_id} className="text-slate-300 hover:bg-white/[0.02]" data-testid={`ramrisk-row-${s.submission_id}`}>
                          ...
                          <td className="px-4 py-2.5">
                            <button
                              onClick={() => navigate(`/ram/risk-map/submissions/${s.submission_id}`)}
                              data-testid={`ramrisk-view-${s.submission_id}`}
